...
//            'middleware'    => ['web', 'admin'],
            'middleware' => [
                'admin.auth',"admin.pjax","admin.log","admin.permission","admin.bootstrap"
            ],

        ];

        app()->group($attributes, function ($router) {
            $attributes = ['middleware' => 'admin.permission:allow,administrator'];

            /* @var \Illuminate\Routing\Router $router */
            $router->group($attributes, function ($router) {
                resource($router,'auth/users', 'UserController');
                resource($router,'auth/roles', 'RoleController');
                resource($router,'auth/permissions', 'PermissionController');
                resource($router,'auth/menu', 'MenuController', ['except' => ['create']]);
                resource($router,'auth/logs', 'LogController', ['only' => ['index', 'destroy']]);
            });
